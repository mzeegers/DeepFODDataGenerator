...
# This script carries out the MSD network training using the workflow-generated (experimental) training data with 'many' foreign objects (5-8) per object
...
#Select all instances and names of folders containing CT scan data (with many foreign objects)
flsinFolders = []
flstgFolders = []
flsinFoldersPossible = sorted(os.listdir(DataPath + Dataset + '/'))[111:131+len(NoFO)]
flstgFoldersPossible = sorted(os.listdir(DataPath + GT + '/'))[111:131+len(NoFO)]
PossibleIntegers = list(range(0,len(sorted(os.listdir(DataPath + Dataset + '/'))[111:131+len(NoFO)])))
