...
# This script carries out the UNet network training using the workflow-generated (experimental) training data with one radiograph per object
...
## Create list of files for training set
flsin = []
flstg = []
with open('TrainSettingsFilesTrainingNumObj' + str(NumObj) + 'Run' + str(Run) +'.txt', 'r') as f:
    line = f.readline()
    for line in f:
        filename = line[0:-1]
...
model = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=30000000) #Many more epochs because an epoch is faster with only single radiographs
